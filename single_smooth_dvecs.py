...
def compute_dvec_from_file(path, fwhm_arcmin, baryon_feedback, physical_scale_mpc, use_bnt=False):
    ...
    kappa_maps = sim.smoothing_one_scale(kappa_maps, physical_scale_mpc)
...
def compute_dvecs(base_dir, fwhm_arcmin, baryon_feedback, physical_scale_mpc, use_bnt=False):
    ...
    dvec_path = f"dvecs/no_bary/dvecs_single_b{int(baryon_feedback)}_{smooth_tag}_{bnt_tag}.npz"
    np.savez_compressed(
        dvec_path,
        fid=dvecs_fid,
